...
#define LE_DEMO_DEVICE_NAME                        "TagStation(Master)"
...
//      Result = GAP_LE_Perform_Scan(BluetoothStackID, stActive, 10, 10, latPublic, fpNoFilter, TRUE, GAP_LE_Event_Callback, 0);
      Result = GAP_LE_Perform_Scan(BluetoothStackID, stActive, 100, 70, latPublic, fpNoFilter, TRUE, GAP_LE_Event_Callback, 0);
      ...
      if((LEConnectionIndex = FindFreeLEIndex()) >= 0)
      {
          ...
          if(FindLEIndexByAddress(BD_ADDR) == -1)
          {
            ...
            if(Result >= 0)
            {
               ...
               /* make the connection.                                  */
               Result = GAP_LE_Create_Connection(BluetoothStackID, 100, 70, Result?fpNoFilter:fpWhiteList, Address_Type, Result?&BD_ADDR:NULL, latPublic, &ConnectionParameters, GAP_LE_Event_Callback, 0);
...
static int AdvertiseLE(ParameterList_t *TempParam)
{
  ...
  unsigned char                       AdData[31];
  BTPS_MemInitialize(&(AdData), 0, sizeof(AdData));
  if ( (TempParam) && (TempParam->NumberofParameters > 1) )
  {
      if ( TempParam->Params[0].intParam == 1 )
      {
          ...
          {
/*
              AdData[0] = 0x1E;    // size 25=0x19 30=0x1E
              AdData[1] = 0x11;    //0 DFI
              AdData[5] = 0x00;    //0 DID
              AdData[9] = 0x0F;    //0 SID
              AdData[18] = 0x01;   //1 920/*BLE
              AdData[19] = 0xB8;   //0 rssi:-72
              AdData[22] = 0x01;   //1 TagID 0x100=256        
              AdData[23] = 0x00;   //0 TagID 0x100=256
              AdData[25] = 0x64;   //1 Temp1:100
              AdData[27] = 0xC8;   //1 Temp2:200
              AdData[28] = 0x1;    //1 Vibration:300
              AdData[29] = 0x2C;   //1 Vibration:300
              AdData[30] = 0x00;   // parity
*/
              AdData[0] = 0x19;    // size 25=0x19 30=0x1E
              AdData[1] = 0x11;    //0 DFI
              AdData[9] = 0xF;     //0 SID
              AdData[10] = 0x1;    //1 920/*BLE
              AdData[11] = 0xB8;   //0 rssi:-72
              AdData[14] = 0x01;   //0 TagID
              AdData[15] = 0x00;   //0 TagID
              AdData[17] = 0x64;   //1 Temp1:100
              AdData[19] = 0xC8;   //1 Temp2:200
              AdData[20] = 0x1;    //1 Vibration:300
              AdData[21] = 0x2C;   //1 Vibration:300
              AdData[22] = 0x00;   //0 H/W
              AdData[23] = 0x00;   //0 S/W
              AdData[24] = 0x00;   //0 Region
              AdData[25] = 0x01;   // parity
          }
          else{
              AdData[0] = 0x19;    // size 25=0x19 30=0x1E
              AdData[1] = 0x12;    //0 DFI
...
//            }
            ...
            for ( int i = 1; i < 31; i++ )
            {
                Advertisement_Data_Buffer.AdvertisingData.Advertising_Data[i] = AdData[i];
            }
            
            /* Write thee advertising data to the chip.                 */            
            ret_val = GAP_LE_Set_Advertising_Data
              (BluetoothStackID, 
...
*/
               // nishiyama
               Advertisement_Data_Buffer.ScanResponseData.Scan_Response_Data[0] = 30;
               for ( int i = 1; i < 31; i++ )
               {
                   Advertisement_Data_Buffer.ScanResponseData.Scan_Response_Data[i] = AdData[i];
               }
               
               ret_val = GAP_LE_Set_Scan_Response_Data
                 (BluetoothStackID, 
...
//                  AdvertisingParameters.Advertising_Interval_Max  = 200;
                  AdvertisingParameters.Advertising_Interval_Min  = 20;
                  AdvertisingParameters.Advertising_Interval_Max  = 40;
                  ...
                  ret_val = GAP_LE_Advertising_Enable
                    (BluetoothStackID, 
                     TRUE, 
                     &AdvertisingParameters, 
                     &ConnectabilityParameters, 
...
//               DisplayAdvertisingData(&(DeviceEntryPtr->Advertising_Data));
               ...
               // nishiyama
               Display(("%02X", (int)(DeviceEntryPtr->RSSI & 0xff)));
               Display(("%02X", (int)(DeviceEntryPtr->Raw_Report_Length & 0xff)));
               if ( DeviceEntryPtr->Raw_Report_Length == 0x1F )
               {
                 DeviceEntryPtr->Raw_Report_Length = DeviceEntryPtr->Raw_Report_Length;
               }
               ...
               if(DeviceEntryPtr)
               {
                  for(Index = 0; Index < DeviceEntryPtr->Advertising_Data.Number_Data_Entries; Index++)
                  {
                      Display(("%02X", DeviceEntryPtr->Advertising_Data.Data_Entries[Index].AD_Type));
                      if(DeviceEntryPtr->Advertising_Data.Data_Entries[Index].AD_Data_Buffer)
                      {
                        for(Index2 = 0; Index2 < DeviceEntryPtr->Advertising_Data.Data_Entries[Index].AD_Data_Length; Index2++)
                        {
                           Display(("%02X", DeviceEntryPtr->Advertising_Data.Data_Entries[Index].AD_Data_Buffer[Index2]));
                        }
                      }
                  }
               }
